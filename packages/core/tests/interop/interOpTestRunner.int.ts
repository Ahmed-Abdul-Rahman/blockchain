...
describe.skip('Interop - Data Replication Tests', () => {
...
describe.skip('Interop - DHT Routing and Iterative Fetching', () => {
...
describe('Interop - Data Convergence (Anti-Entropy) Tests', () => {
  it.skip(`should converge a late-joining peer's store via background anti-entropy sync`, async () => {
  ...
  it.skip(`should converge a revived (dropped then restored) peer's store via background anti-entropy sync`, async () => {
    const totalNodes = totalNodesArg ?? 12;
    const runDurationSec = runDurationSecArg ?? 300;
    const messageRate = messageRateArg ?? 5;
    const pubsubTopic = pubsubTopicArg ?? '/bench/1';
