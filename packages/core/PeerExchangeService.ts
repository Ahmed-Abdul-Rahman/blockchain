...
type PexMsg =
  ...
  | { type: 'PEER_LIST'; peers: PeerInfoLite[] }
  | { type: 'PEX_GOSSIP'; peers: PeerInfoLite[]; ts: number; origin?: string };
  ...
  private async loopGossip() {
    while (!this.stopped) {
      ...
      try {
        ...
        if (peers.length) {
          const msg: PexMsg = { type: 'PEX_GOSSIP', peers, ts: now(), origin: this.node.peerId.toString() };
