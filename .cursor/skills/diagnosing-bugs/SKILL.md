...
Read `.cursor/PROJECT.md` for DeChat-specific commands and architecture anchors.

## DeChat feedback loops (try these first)

| Bug domain | Loop | Speed |
|------------|------|-------|
...
**Build-first rule:** integration tests run compiled JS — always `yarn build` before `yarn test:int`.

**P2P bugs to watch for:** auth bypass (peer in registry before handshake), connection storms (bypassing `DialQueue`), missing boot lock (`replicaStore.init()` before libp2p start).
