...
For DeChat today (no web UI yet): **default to the LOGIC branch**. Place prototypes under `packages/core/src/` or `packages/core/tests/` with a `PROTOTYPE_` prefix in the filename. Run with `tsx` or `yarn core test:file` depending on shape. For P2P behaviour questions, prefer a minimal worker-thread harness based on `configureNode()` in `workerUitls.ts` over mocking libp2p.
